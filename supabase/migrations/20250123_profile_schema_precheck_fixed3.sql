...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Profiles table does not exist! Run create_profiles_table.sql first.';
    END IF;
    ...
    IF NOT column_exists THEN
        RAISE EXCEPTION 'Column full_name does not exist in profiles table! Run create_profiles_table.sql first.';
    END IF;
END $$;
...
BEGIN
    ...
    FROM public.profiles WHERE full_name IS NOT NULL AND char_length(full_name) < 1;
